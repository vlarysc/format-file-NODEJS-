...
let columnsUnformatted = ["old_id", "coordenador"];
...
const formatColumns = async () => {
  try {
    ...
    columnsConverting.toLowerCase().includes("consultor is") ? columns.push('consultorLead') : '';
...
formatColumns().then(total => {
  ...
  formatCsv("lead", "CRED-5801", "leads", settingsOfColumns, columnsProcessed)
})